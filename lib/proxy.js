                ...
                authenticator.authenticate(authObj).then(
                    function(useSessionObj) {
                        ...
                        if(testing) {
                            ...
                            // User is authenticated and the VM should be up and running
                            // with attached storage
                            vmSocket.connect(settings.vm_port, useSessionObj.user.vm_ip, function () {
                                winston.info('User: \'%s\' connected to VM: %s', useSessionObj.user.username, useSessionObj.user.vm_ip.vm_ip);
...
//            }
    ...
    /**
     * When the client closes, close the VM connection
     *
     */
