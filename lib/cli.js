...
program
    ...
    .action(function (un) {
        ...
        /*users.findUser({username: un})
            .then(
            function (user) {
                Openstack.createUserVolume(user.username, function (err1, vol) {
                    if (err1) {
                        console.log("        ERROR creating volume for: ", user.username, " Reason: ", err1);
                        mongoose.connection.close();
                    } else {
                        console.log("        Volume created for: ", user.username, " Vol ID: ", vol.id);
                        console.log("        ... saving information to User's account ...");
                        user.volume_id = vol.id;
                        user.save(function (err2, updateduser) {
                            if (updateduser) {
                                console.log("        Success: volume info. saved to User's account");
                                mongoose.connection.close();
                            } else {
                                console.log("        ERROR saving volume info to User's account: ", err2);
                                mongoose.connection.close();
                            }
                        });
                    }
