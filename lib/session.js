...
exports.create = function (obj) {
    ...
    var s = new SessionModel({sid: uidhelper(UID_SIZE), username: obj.username,
    ...
    s.save(function(err, sess){
        if(err) { 
            ...
            SessionModel.remove({ username: obj.username, sid: { $ne: sess.sid }}, function(err, result) {
                if (err) {
        ...
        if(sess) {
            ...
            if (!expired && !timedOut) {
                ...
                sess.save(function(err, updated) {
                    if (updated)
                        deferred.resolve({session: updated, username: updated.username});
                    else
